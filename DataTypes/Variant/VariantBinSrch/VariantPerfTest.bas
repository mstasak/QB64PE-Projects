...
For i = 1 To BLOCKSIZE
    's = VTStr(vars(i))
    VTRelease vars(randomMap(i))
Next i
VTDump
timings(testIx + 1) = Timer
Print "Random order release all (one at a time) "; BLOCKSIZE; " String variants: "; timings(testIx + 1) - timings(testIx); " seconds."
testIx = testIx + 2
End
